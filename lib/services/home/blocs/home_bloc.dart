...
class HomeBloc extends Bloc<AppEvent,AppState>{
  static HomeBloc get instance => BlocProvider.of(RouterGenerator.navigatorState.currentContext!);
  ...
  Stream<AppState> mapEventToState(AppEvent event) async* {
    ...
    try{
      if(event is GetData){
        List<PostModel> data = await PostRepo.getPosts();
        if(data.isNotEmpty) {yield Done(cards: List.generate(data.length, (index) => BookHomeWidget(model: data[index])));}
        else {yield Empty();}
      }
    }
    on FirebaseException catch (e){
      AppCore.showSnackBar(notification: AppNotification(message: e.message!,backgroundColor: AppColor.REJECTED));
    }
  }
}